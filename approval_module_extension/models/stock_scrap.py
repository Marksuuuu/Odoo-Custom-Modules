...
class StockScrap(models.Model):
    _name = "stock.scrap"
    _inherit = ['stock.scrap', 'department.approvers']
        ...
        for rec in self:
            if rec.approval_status == 'approved' or rec.state == 'approved':
                ...
                res = self.env["department.approvers"].search(
                    [("dept_name", "=", rec.department_id.id), ("approval_type.name", '=', 'Stock Scrap')])
    ...
    def _compute_approver_count(self):
        ...
        for record in self:
            department_approvers = self.env['department.approvers'].search(
                [('dept_name', '=', record.department_id.id), ("approval_type.name", '=', 'Stock Scrap')])
            count = 0
    ...
    def compute_check_status(self):
        """
        When installing stock_scrap in method compute_check_status comment out the for loop first.
    ...
    def generate_wiv_link(self):
        action = self.env['ir.actions.act_window'].search([('res_model', '=', 'stock.scrap')], limit=1)
        # search_wiv = self.env['stock.scrap.type'].search([('name', '=', 'WIV Request')])
    ...
    def sendingEmail(self, fetch_getEmailReceiver, po_form_link, approval_list_view_url):
        ...
        approval_url = "{}/stock_scrap/request/approve/{}".format(base_url, token)
        disapproval_url = "{}/stock_scrap/request/disapprove/{}".format(base_url, token)
        ...
        msg['Subject'] = 'Scrap Order For Approval [' + self.name + ']'
        ...
        html_content += f"""
                                    ...
                                    <dd>Owner: &nbsp;&nbsp;{self.owner_id.name if self.origin != False else ""}</dd>
                                    <br></br>
        ...
        html_content += f"""
                                        <tr>
                                            <td>{self.product_id.name}</td>
                                            <td>{self.product_id.name + '[' + self.product_id.name + ']'}</td>
                                            <td>{self.scrap_qty} | {self.product_uom_id.name}</td>
                                            <td>{self.origin}</td>
                                        </tr>
                                    """
    ...
    def send_disapproval_email(self, recipient_list, po_form_link):
        ...
        msg['Subject'] = 'Scrap Order Disapproved [' + self.name + ']'
    ...
    def send_to_final_approver_email(self, recipient_list):
        ...
        wiv_url = "{}/stock_scrap/request/wiv/{}".format(base_url, token)
        dashboard_url = "{}/stock_scrap/request/dashboard/{}".format(base_url, token)

        msg = MIMEMultipart()
        msg['From'] = formataddr(('Odoo Mailer', sender))
        msg['To'] = ', '.join(recipient_list)
        msg['Subject'] = 'Scrap Order For Approved [' + self.name + ']'
        ...
        html_content += f"""
                            ...
                            <a href="{wiv_url}" style='background-color: blue;
                                ...
                                padding: 10px 24px;
                                border-radius: 50px;
                                cursor: pointer;'>ODOO SCRAP FORM</a>
