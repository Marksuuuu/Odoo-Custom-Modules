...
class ClientPickUpPermitFormController(http.Controller):
    @http.route('/dex_form_request_approval/request/cpp_approve/<string:token>', type='http', auth='public', website=True)
    def cpp_approval(self, token):
        request_form = request.env['client.pick.up.permit'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/cpp_disapprove/<string:token>', type='http', auth='public',
                website=True,
                csrf=False,
                method=['GET', 'POST'])
    def cpp_disapproval(self, token, **post):
        request_form = request.env['client.pick.up.permit'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/cpp_dashboard/<string:token>', type='http', auth='public', website=True)
    def cpp_dashboard(self, token):
        request_form = request.env['client.pick.up.permit'].sudo().search([('approval_link', '=', token)])
