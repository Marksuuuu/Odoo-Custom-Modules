...
<body>
  ...
  <h1 class="error-title">
    Woops! <br>Something went wrong Invalid Link! :(
  </h1>
  <h2 class="error-subtitle">
   Have you tried Checking it and on again? or Else it is Currently Approved!
  </h2>
...
class GasolineAllowanceFormApproval(http.Controller):
    @http.route('/dex_form_request_approval/request/gaf_report_download/<string:token>', type='http', auth='public',
                website=True)
    def gaf_pdf_redirect(self, token):
        request_form = request.env['gasoline.allowance.form'].sudo().search([('approval_link', '=', token)])
        if request_form:
            request_form.print_form()
            # msg = "success!"
            # return f"""<script>alert("{msg}");window.close();</script>"""
        else:
            return INVALID_LINK

    @http.route('/dex_form_request_approval/request/gaf_approve/<string:token>', type='http', auth='public', website=True)
    def gaf_approval(self, token):
        request_form = request.env['gasoline.allowance.form'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/gaf_disapprove/<string:token>', type='http', auth='public',
                website=True,
                csrf=False,
                method=['GET', 'POST'])
    def gaf_disapproval(self, token, **post):
        request_form = request.env['gasoline.allowance.form'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/gaf_dashboard/<string:token>', type='http', auth='public', website=True)
    def gaf_dashboard(self, token):
        request_form = request.env['gasoline.allowance.form'].sudo().search([('approval_link', '=', token)])
            ...
            return f"""
                                <script>
                                    (function() {{
                                        ...
                                        window.location.href = "{request_form.approval_dashboard_link() if request_form.approval_dashboard_link() else ''}";
                                    }})();
                                </script>
                            """
