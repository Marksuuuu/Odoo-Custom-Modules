...
class OfficialBusinessFormController(http.Controller):
    @http.route('/dex_form_request_approval/request/obf_approve/<string:token>', type='http', auth='public', website=True)
    def obf_request_approval(self, token):
        request_form = request.env['official.business.form'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/obf_disapprove/<string:token>', type='http', auth='public',
                website=True,
                csrf=False,
                method=['GET', 'POST'])
    def obf_request_disapproval(self, token, **post):
        request_form = request.env['official.business.form'].sudo().search([('approval_link', '=', token)])
        print(request_form, 'testttt')
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/obf_dashboard/<string:token>', type='http', auth='public', website=True)
    def obf_dashboard(self, token):
        request_form = request.env['official.business.form'].sudo().search([('approval_link', '=', token)])
        user = request.env.user
        print('go in obf', user)
        if request_form:
