...
class OnlinePurchaseFormApproval(http.Controller):
    @http.route('/dex_form_request_approval/request/opf_approve/<string:token>', type='http', auth='public', website=True)
    def opf_approval(self, token):
        request_form = request.env['on.line.purchases'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/opf_disapprove/<string:token>', type='http', auth='public',
                website=True,
                csrf=False,
                method=['GET', 'POST'])
    def opf_disapproval(self, token, **post):
        request_form = request.env['on.line.purchases'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/opf_dashboard/<string:token>', type='http', auth='public', website=True)
    def opf_dashboard(self, token):
        request_form = request.env['on.line.purchases'].sudo().search([('approval_link', '=', token)])
