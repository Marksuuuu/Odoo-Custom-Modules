...
class OvertimeAuthorizationFormApproval(http.Controller):
    @http.route('/dex_form_request_approval/request/oaf_approve/<string:token>', type='http', auth='public', website=True)
    def oaf_approval(self, token):
        request_form = request.env['overtime.authorization.form'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/oaf_disapprove/<string:token>', type='http', auth='public',
                website=True,
                csrf=False,
                method=['GET', 'POST'])
    def oaf_disapproval(self, token, **post):
        request_form = request.env['overtime.authorization.form'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/oaf_dashboard/<string:token>', type='http', auth='public', website=True)
    def oaf_dashboard(self, token):
        request_form = request.env['overtime.authorization.form'].sudo().search([('approval_link', '=', token)])
