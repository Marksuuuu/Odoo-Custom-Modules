...
class PaymentRequestFormApproval(http.Controller):
    @http.route('/dex_form_request_approval/request/prf_approve/<string:token>', type='http', auth='public',
                website=True)
    def prf_approval(self, token):
        request_form = request.env['payment.request.form'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/prf_disapprove/<string:token>', type='http', auth='public',
                website=True,
                csrf=False,
                method=['GET', 'POST'])
    def prf_disapproval(self, token, **post):
        request_form = request.env['payment.request.form'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/prf_dashboard/<string:token>', type='http', auth='public',
                website=True)
    def prf_dashboard(self, token):
        request_form = request.env['payment.request.form'].sudo().search([('approval_link', '=', token)])
