...
class RequestForCashAdvanceFormApproval(http.Controller):
    @http.route('/dex_form_request_approval/request/rfca_approve/<string:token>', type='http', auth='public',
                website=True)
    def rfca_approval(self, token):
        request_form = request.env['request.for.cash.advance'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/rfca_disapprove/<string:token>', type='http', auth='public',
                website=True,
                csrf=False,
                method=['GET', 'POST'])
    def rfca_disapproval(self, token, **post):
        request_form = request.env['request.for.cash.advance'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/rfca_dashboard/<string:token>', type='http', auth='public',
                website=True)
    def rfca_dashboard(self, token):
        request_form = request.env['request.for.cash.advance'].sudo().search([('approval_link', '=', token)])
