...
class TransportNetworkVehicleFormApproval(http.Controller):
    @http.route('/dex_form_request_approval/request/tnvf_approve/<string:token>', type='http', auth='public',
                website=True)
    def tnvf_approval(self, token):
        request_form = request.env['transport.network.vehicle.form'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/tnvf_disapprove/<string:token>', type='http', auth='public',
                website=True,
                csrf=False,
                method=['GET', 'POST'])
    def tnvf_disapproval(self, token, **post):
        request_form = request.env['transport.network.vehicle.form'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_approval/request/tnvf_dashboard/<string:token>', type='http', auth='public',
                website=True)
    def tnvf_dashboard(self, token):
        request_form = request.env['transport.network.vehicle.form'].sudo().search([('approval_link', '=', token)])
