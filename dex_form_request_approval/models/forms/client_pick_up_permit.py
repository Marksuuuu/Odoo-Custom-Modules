...
class ClientPickupPermit(models.Model):
    _name = 'client.pick.up.permit'
    ...
    def _get_department_domain(self):
        approval_types = self.env['approver.setup'].search([('approval_type', '=', 'client_pickup')])
        if approval_types:
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('create.sequence.form.sequence.cpp') or '/'
        return super(ClientPickupPermit, self).create(vals)
    ...
    def submit_for_approval(self):
        ...
        odoo_action = self.env['ir.actions.act_window'].search([('res_model', '=', 'client.pick.up.permit')], limit=1)
    ...
    def send_to_final_approver_email_disapproved(self, recipient_list):
        ...
        html_content += f"""
                                ...
                                <div class="item-details">
                                    <p>Supplier : {self.supplier_id.name if self.supplier_id else ''}</p>
                                    <p>Invoice or Reference: {self.invoice_or_ref if self.invoice_or_ref else ''}</p>
                                </div>
                                </hr>
                                </div>
    ...
    def send_to_final_approver_email(self, recipient_list):
        ...
        msg['To'] = ', '.join(recipient_list)
        msg[
            'Subject'] = f"{re.sub(r'[-_]', ' ', self.form_request_type).title() if self.approval_status else ''} Request has been [{re.sub(r'[-_]', ' ', self.form_request_type).title() if self.approval_status else ''}] [{self.name}]"
    ...
    def generate_odoo_link(self):
        # Generate Odoo Link Section
        action = self.env['ir.actions.act_window'].search([('res_model', '=', 'client.pick.up.permit')], limit=1)
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
