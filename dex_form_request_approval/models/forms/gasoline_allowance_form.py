...
class GasolineAllowanceForm(models.Model):
    ...
    def _onchange_requesters_id(self):
        if self.requesters_id and self.requesters_id.department_id:
            ...
            approval_type = 'gasoline_allowance'
    ...
    def _onchange_one2many_field(self):
        if not self.gaf_lines:
            raise UserError("Please note that data must be provided in the required fields to proceed.")

    @api.model
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('create.sequence.form.sequence.gaf') or '/'
            # self._onchange_one2many_field()
        return super(GasolineAllowanceForm, self).create(vals)

    def write(self, values):
        res = super(GasolineAllowanceForm, self).write(values)
        # self._onchange_one2many_field()
        return res
    ...
    def total_gas(self):
        ...
        for record in self:
            estimated_km_values = [line._estimated_km for line in record.gaf_lines]
            total = sum(estimated_km_values)
        return total

    @api.depends('total_gasoline_allowance')
    ...
    def submit_for_approval(self):
        ...
        approval_action = self.env['ir.actions.act_window'].search(
            [('name', '=', 'Gasoline Allowance Form')], limit=1)
        ...
        odoo_action = self.env['ir.actions.act_window'].search([('res_model', '=', 'gasoline.allowance.form')], limit=1)
        ...
        html_content += f""" 
                                    <div class="container">
                                        ...
                                        <div class="item-details">
                                            ...
                                            <p>Reason for {re.sub(r'[-_]', ' ', self.approval_status).title() if self.approval_status else '' if self.approval_status == 'disapprove' else (re.sub(r'[-_]', ' ', self.approval_status).title() if self.approval_status else '' if self.approval_status == 'cancel' else '')}: {self.disapproval_reason if self.disapproval_reason else ''}</p>
    ...
    def send_to_final_approver_email(self, recipient_list):
        ...
        approval_url = "{}/dex_form_request_approval/request/gaf_approve/{}".format(base_url, token)
        disapproval_url = "{}/dex_form_request_approval/request/gaf_disapprove/{}".format(base_url, token)
    ...
    def generate_odoo_link(self):
        # Generate Odoo Link Section
        action = self.env['ir.actions.act_window'].search([('res_model', '=', 'gasoline.allowance.form')], limit=1)
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
...
class GasolineAllowanceFormLines(models.Model):
    _name = 'gasoline.allowance.form.lines'
    _description = 'Overtime Authorization Form Lines'

    gaf_connection = fields.Many2one('gasoline.allowance.form', string='Connection')
    ...
    _estimated_km = fields.Float(string='Estimated KM')
