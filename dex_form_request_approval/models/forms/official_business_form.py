...
class OfficialBusinessForm(models.Model):
    _name = 'official.business.form'
    _inherit = 'approval.fields.plugins'

    obf_lines = fields.One2many('official.business.form.lines', 'obf_connection', string='Official Business Form Lines')
    ...
    def _onchange_requesters_id(self):
        if self.requesters_id and self.requesters_id.department_id:
            ...
            approval_type = 'official_business'
    ...
    def _onchange_department_id(self):
        for rec in self.department_id:
            if rec.approval_type == 'official_business':
                print(rec.dept_name.name)

    def print_form(self):
        return self.env.ref('dex_form_request_approval.obf_report_id').report_action(self)

    # @api.onchange('obf_lines')
    def _onchange_one2many_field(self):
        if not self.obf_lines:
            raise UserError("Please note that data must be provided in the required fields to proceed.")

    @api.model
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('create.sequence.form.sequence.obf') or '/'

        record = super(OfficialBusinessForm, self).create(vals)
        # record._onchange_one2many_field()
        return record

    def write(self, values):
        res = super(OfficialBusinessForm, self).write(values)
    ...
    def submit_for_approval(self):
        ...
        approval_action = self.env['ir.actions.act_window'].search(
            [('name', '=', 'Official Business Form')], limit=1)
        ...
        odoo_action = self.env['ir.actions.act_window'].search([('res_model', '=', 'official.business.form')], limit=1)
    ...
    def send_to_final_approver_email(self, recipient_list):
        ...
        approval_url = "{}/dex_form_request_approval/request/obf_approve/{}".format(base_url, token)
    ...
    def generate_odoo_link(self):
        # Generate Odoo Link Section
        action = self.env['ir.actions.act_window'].search([('res_model', '=', 'official.business.form')], limit=1)
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
...
class OfficialBusinessFormLines(models.Model):
    _name = 'official.business.form.lines'
