...
class OvertimeAuthorizationForm(models.Model):
    _name = 'overtime.authorization.form'
    _inherit = 'approval.fields.plugins'

    otaf_lines = fields.One2many('overtime.authorization.form.lines', 'otaf_connection',
    ...
    @api.constrains('otaf_lines')
    def _check_same_day(self):
        for record in self:
            for datetime_pair in record.otaf_lines:
                start = fields.Datetime.from_string(datetime_pair._from)
                end = fields.Datetime.from_string(datetime_pair._to)
                if start.date() != end.date():
                    raise ValidationError("Start and end datetime should be on the same day.")

    def _get_department_domain(self):
        approval_types = self.env['approver.setup'].search([('approval_type', '=', 'overtime_authorization')])
    ...
    def _onchange_requesters_id(self):
        if self.requesters_id and self.requesters_id.department_id:
            ...
            approval_type = 'overtime_authorization'
    ...
    def print_form(self):
        return self.env.ref('dex_form_request_approval.auf_report_id').report_action(self)

    # @api.onchange('otaf_lines')
    # def _onchange_one2many_field(self):
    #     if not self.otaf_lines:
    #         raise UserError("Please note that data must be provided in the required fields to proceed.")

    @api.model
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('create.sequence.form.sequence.oaf') or '/'

        record = super(OvertimeAuthorizationForm, self).create(vals)
        # record._onchange_one2many_field()
        return record

    def write(self, values):
        res = super(OvertimeAuthorizationForm, self).write(values)
    ...
    def submit_for_approval(self):
        ...
        approval_action = self.env['ir.actions.act_window'].search(
            [('name', '=', 'Overtime Authorization Form')], limit=1)
        ...
        odoo_action = self.env['ir.actions.act_window'].search([('res_model', '=', 'overtime.authorization.form')],
                                                               limit=1)
        ...
        odoo_params = {
            ...
            "model": "overtime.authorization.form",
            "view_type": "form",
            "cids": 1,
    ...
    def send_to_final_approver_email(self, recipient_list):
        ...
        approval_url = "{}/dex_form_request_approval/request/oaf_approve/{}".format(base_url, token)
    ...
    def generate_odoo_link(self):
        # Generate Odoo Link Section
        action = self.env['ir.actions.act_window'].search([('res_model', '=', 'overtime.authorization.form')], limit=1)
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
...
class OvertimeAuthorizationFormLines(models.Model):
    _name = 'overtime.authorization.form.lines'
    _description = 'Overtime Authorization Form Lines'

    otaf_connection = fields.Many2one('overtime.authorization.form', string='Connection')
    # _date = fields.Date(string='Date', default=lambda self: datetime.now())
    _from = fields.Datetime(string='From', default=lambda self: datetime.now())
    _to = fields.Datetime(string='To')
    _reason = fields.Char(string='Reason for Overtime')
    _total = fields.Float(string='Total', compute='_compute_total_hours', store=True)

    @api.depends('_from', '_to')
    def _compute_total_hours(self):
        for record in self:
            if record._from and record._to:
                start = fields.Datetime.from_string(record._from)
                end = fields.Datetime.from_string(record._to)
                if start.date() != end.date():
                    raise ValidationError("Start and end datetime should be on the same day.")
