...
class RequestForCashAdvance(models.Model):
    _name = 'request.for.cash.advance'
    _inherit = 'approval.fields.plugins'

    rfca_lines = fields.One2many('request.for.cash.advance.lines', 'rfca_connection',
                                 string='Request For Cash Advance Form Lines')
    ...
    def _get_department_domain(self):
        approval_types = self.env['approver.setup'].search([('approval_type', '=', 'cash_advance')])
    ...
    # @api.onchange('rfca_lines')
    def _onchange_one2many_field(self):
        if not self.rfca_lines:
            raise UserError("Please note that data must be provided in the required fields to proceed.")

    @api.model
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('create.sequence.form.sequence.rfca') or '/'

        record = super(RequestForCashAdvance, self).create(vals)
        # record._onchange_one2many_field()
        return record

    def write(self, values):
        res = super(RequestForCashAdvance, self).write(values)
    ...
    def submit_for_approval(self):
        ...
        approval_action = self.env['ir.actions.act_window'].search(
            [('name', '=', 'Request for Cash Advance')], limit=1)
        ...
        odoo_action = self.env['ir.actions.act_window'].search([('res_model', '=', 'request.for.cash.advance')],
                                                               limit=1)
        ...
        odoo_params = {
            ...
            "model": "request.for.cash.advance",
            "view_type": "form",
            "cids": 1,
    ...
    def send_to_final_approver_email(self, recipient_list):
        ...
        approval_url = "{}/dex_form_request_approval/request/rfca_approve/{}".format(base_url, token)
        disapproval_url = "{}/dex_form_request_approval/request/rfca_disapprove/{}".format(base_url, token)
        ...
        msg['To'] = ', '.join(recipient_list)
        msg[
            'Subject'] = f"{re.sub(r'[-_]', ' ', self.approval_status).title() if self.approval_status else ''} Request has been Approved [{self.name}]"
    ...
    def generate_odoo_link(self):
        # Generate Odoo Link Section
        action = self.env['ir.actions.act_window'].search([('res_model', '=', 'request.for.cash.advance')], limit=1)
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
    ...
    def sending_email_to_next_approver(self, get_all_email_receiver, form_link, approval_list_view_url):
        ...
        msg[
            'Subject'] = f"{re.sub(r'[-_]', ' ', self.approval_status).title() if self.approval_status else ''} For Approval [{self.name}]"
...
class RequestForCashAdvanceLines(models.Model):
    _name = 'request.for.cash.advance.lines'
    _description = 'Online Purchases Lines'

    rfca_connection = fields.Many2one('request.for.cash.advance', string='Connection')
