...
class DexJobRequestFormApproval(http.Controller):
    @http.route('/dex_form_request_form_odoo/request/rfca_approve/<string:token>', type='http', auth='public',
                website=True)
    def jrf_approval(self, token):
        request_form = request.env['job.request'].sudo().search([('approval_link', '=', token)])
        if request_form:
    ...
    @http.route('/dex_form_request_form_odoo/request/rfca_disapprove/<string:token>', type='http', auth='public',
                website=True,
                csrf=False,
                method=['GET', 'POST'])
    def jrf_disapproval(self, token, **post):
        request_form = request.env['job.request'].sudo().search([('approval_link', '=', token)])
        if request_form:
            if request.httprequest.method == 'POST' and 'reason' in post:
                reason = post.get('reason')
                request_form.write(
                    {'state': 'disapprove', 'disapproval_reason': reason})
    ...
    @http.route('/dex_form_request_form_odoo/request/rfca_dashboard/<string:token>', type='http', auth='public',
                website=True)
    def jrf_dashboard(self, token):
        request_form = request.env['job.request'].sudo().search([('approval_link', '=', token)])
