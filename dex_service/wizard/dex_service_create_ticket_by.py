...
class DexServiceCreateTicketBy(models.TransientModel):
    _name = 'dex_service.create.ticket.by'
    ...
    edp_code_not_existing = fields.Many2many('dex_service.no.prod.exist',string='EDP-Code')
    ...
    create_ticket_by_line_ids = fields.One2many('dex_service.create.ticket.by.line', 'create_ticket_by_line_line_ids')
    ...
    def create_service_lines_warranty(self, requests_res):
        ...
        for line_id in self.warranty_number:
            vals = {
                'with_warranty': True,
                'item_description': line_id.edp_code.name,
    ...
    def create_service_lines_invoice(self, requests_res):
        ...
        for line_id in self.create_ticket_by_line_ids: 
            vals = {
                ...
                'invoice_number': (self.invoice_no.invoice_prefix if self.invoice_no.invoice_prefix else '') + '-' + (self.invoice_no.invoice_number if self.invoice_no.invoice_number else ''),
...
class CreateTicketByLine(models.TransientModel):
    _name = 'dex_service.create.ticket.by.line'
    
    
    create_ticket_by_line_line_ids = fields.Many2one('dex_service.create.ticket.by')
